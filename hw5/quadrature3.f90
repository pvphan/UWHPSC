...
real(kind=8) function simpson(f, a, b, n)
    ! Estimate the integral of f(x) from a to b using
    ! Simpsons Rule with n points
    ...
    integer, intent(in) :: n
    real(kind=8) :: h, int_sum, int_simpson, fjprev, fjmid, fjnext
    integer :: j

    h = (b-a)/(n-1.d0)

    ! -rm !$omp parallel do private(fjprev,fjmid,fjnext) reduction(+ : int_sum) 
    do j=1, n-1
        ...
        int_sum = int_sum + (1/6.d0) * (fjprev + 4.d0*fjmid + fjnext)
